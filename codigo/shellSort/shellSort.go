...
var dir string = "output/shell"


func expPassosCSV(passos [][]int){
	arquivo, erro := os.Create(fmt.Sprintf("%s/shellSortPassos.csv", dir))
	...
	for _, valor := range passos {
	    erro := escrever.Write([]string {fmt.Sprintf("%d", valor[0]), fmt.Sprintf("%d", valor[1])})
